...
function displayPart
{
   ...
   Write-Host ("-".PadLeft($size,"-"))
   Write-Host ((" {0} " -f $title).PadLeft($size/2+([Math]::Floor($title.length/2)),"-").PadRight($size,"-"))
   Write-Host ("-".PadLeft($size,"-"))
}
...
ForEach($user in $users)
{
   ...
   if(!(Test-Path -Path $pathToFolder))
   {
      ...
      catch
      {
         ...
         Write-Host ("User account for '{0}' not found in Active Directory" -f $user.username) 
         Continue
      }
      
   
      Write-host ("Folder for user '{0}' ({1}) doesn't exists -> creation" -f $user.username, $server) 
...
Write-Host ("{0} user(s) folder(s) created" -f $nbCreated) 
...
{  
   ...
   Write-Host ("{0} users found with quota of {1}MB" -f $users.Count, $userStatusToQuota[$userStatus]) 
}

# Envoi du fichier contenant les infos sur le serveur Web.
Write-Host ("Pushing file '{0}' on ditex-web..." -f $quotaUpdateCSV) 
...
Write-Host ("{0} users to delete" -f $users.Count) 

# Envoi du fichier contenant les infos sur le serveur Web.
Write-Host ("Pushing file '{0}' on ditex-web..." -f $userDeleteCSV) 
# !! Si ça bloque sur cette commande dans le fichier log, voir entête du fichier, section "Remarque"
pushFile -targetFolder "/www/mynas/web/upload/mynas_user_delete_request/" -fileToPush $userDeleteCSV
 
# Déclenchement de l'import des données dans la DB (avec vidage de la table juste avant)
Write-Host ("Triggering import for pushed file ({0})..." -f $userDeleteCSV)
